...
O **Konekta** é uma plataforma móvel e web para aproximar **clientes** e **profissionais** em Moçambique: quem procura serviços caseiros encontra quem está perto; quem oferece trabalho encontra oportunidades. O projeto usa **Expo/React Native** para Android, iOS e Web a partir de uma única base de código.
...
- **Base de Dados & Auth:** Supabase (Authentication, PostgreSQL, Storage).
...
/mobile
  ...
  ├── services/             # Integrações (Supabase, Notificações, Storage)
...
2.  **Gestão de Perfis:** Edição de dados, biografia, seleção de avatares e upload de fotografias reais para o Supabase Storage.
...
5.  **Chat em Tempo Real:** Sistema de mensagens diretas entre candidatos e empregadores via Supabase Realtime.
...
2.  **Ambiente:** Cria um ficheiro `.env` com as chaves do Supabase (URL e Anon Key).
...
## 🌍 Missão Social
O Konekta foca-se em ligar oferta e procura de trabalho informal e técnico, com perfis, avaliações e histórico transparente para quem contrata e para quem presta o serviço.
